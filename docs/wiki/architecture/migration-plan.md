...
## Source & donor locations (local machine)

Абстрактні донори з цього плану мають конкретне розташування на машині Anton'а,
**де й працює оркестратор** (Claude Code читає їх напряму через `view`/`read`):

| Донор | Роль | Локальний шлях |
|---|---|---|
| Старий live-сайт (`lnnemml/ISRIB`) | легасі HTML/копі, старий URL-список, реальні ассети | `/home/laptop/Documents/ISRIB/isrib shop website/ISRIB` |
| `nootropics` / NORA (референс-архітектура) | еталон структури, patterns, class-строк, бізнес-логіки | `/home/laptop/Documents/NORA` |

**Як користуватись у спеках/промптах:** коли треба «подивись, як зроблено/написано
X» — посилайся на ці шляхи явно (напр. «match the old hero copy at
`…/ISRIB/…`», «mirror `NORA/src/lib/nowpayments.ts`»), а не переказуй по пам'яті.

**Важливо:** це **machine-local** посилання — їх нема в репо і в git; вони працюють
лише на цій машині. Для single-operator сетапу це ок і прагматично.

**Реконсайл дірки `docs/raw/legacy/`** (яку я флагав на Day 0): її **не треба
наповнювати** копіюванням легасі всередину репо. Оркестратор читає стару верстку
прямо з локального шляху вище. Практичні наслідки:
- **Ассети** (formula SVG, NMR-спектри, FID/COA — те, що показало биті боксики на
  kitchen-sink) копіюються зі старого сайту в `public/` на content-port, не
  вигадуються.
- **Старий URL-список** для 301-редіректів (Session 4.1) енумерується з локальної
  папки старого сайту, а не з `raw/legacy/`.
